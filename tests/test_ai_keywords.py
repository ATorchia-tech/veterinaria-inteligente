...
def test_classify_turnos():
    res = client.post(
        "/ai/classify", json={"text": "Quiero agendar un turno para mañana"}
    )
...
def test_classify_vacunacion():
    res = client.post(
        "/ai/classify", json={"text": "Necesito el refuerzo de la vacuna antirrábica"}
    )
    assert res.status_code == 200
    data = res.json()
...
def test_classify_emergencia():
    res = client.post(
        "/ai/classify", json={"text": "Es una emergencia, está sangrando"}
    )
    assert res.status_code == 200
    data = res.json()
...
def test_classify_precios():
    res = client.post(
        "/ai/classify", json={"text": "¿Cuál es el precio de la castración?"}
    )
    assert res.status_code == 200
    data = res.json()
...
def test_classify_horarios():
    res = client.post(
        "/ai/classify", json={"text": "¿Qué horarios tienen los domingos?"}
    )
    assert res.status_code == 200
    data = res.json()
